...
import com.github.pvb.Ast._
import com.github.pvb.Type._
import com.github.pvb.macro_peg.Parser.Fragment
...
class Parser extends Processor[String, Program, InteractiveSession] {
  object Core extends SCombinator with macro_peg.Parser.Fragment {
    object Klassic {
      ...
      val SUB:      Parser[String] = kwToken("Sub")
      val MUTABLE: Parser[String] = kwToken("mutable")
      val DIM: Parser[String] = kwToken("Dim")
      ...
      //line ::= expression | valDeclaration | functionDefinition | subRolutineDefinition
      lazy val line: Parser[Ast.Node] = rule(variantDeclaration | expression | valDeclaration | functionDefinition | subRoutineDefinition)
      ...
      lazy val ifExpression: Parser[Ast.Node] = rule {
        (%% << CL(IF)) ~ commit(expression ~ CL(THEN) ~ lines ~ CL(ELSE) ~ lines << CL(END) << IF) ^^ {
          case location ~ (condition ~ _ ~ positive ~ _ ~ negative) => IfExpression(location, condition, positive, negative)
        }
      }

      //whileExpression ::= "While" "(" expression ")" expression "End" "While"
      lazy val whileExpression: Parser[Ast.Node] = rule {
        (%% << CL(WHILE)) ~ commit(expression ~ (TERMINATOR >> lines) << CL(END) << WHILE) ^^ {
          case location ~ (condition ~ body) => WhileExpression(location, condition, body)
        }
      }

      //foreachExpression ::= "Foreach" ident "in" expression ")" expression "End" "Foreach"
      lazy val foreachExpression: Parser[Ast.Node] = rule {
        (%% << CL(FOREACH)) ~ commit((CL(ident) << CL(IN)) ~ expression ~ (TERMINATOR >> lines) << CL(END) << FOREACH) ^^ {
      ...
      // valDeclaration ::= "Dim" ident "=" expression
      lazy val valDeclaration: Parser[ValDeclaration] = rule((%% << DIM) ~ commit(ident ~ (typeAnnotation.? << CL(EQ)) ~ expression) ^^ {
      ...
      lazy val functionDefinition: Parser[FunctionDefinition] = rule {
        (%% << CL(FUNCTION)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ typeAnnotation.? ~ lines) << CL(END) << FUNCTION ^^ {
      ...
      // subRoutineDefinition ::= "Sub" ident  ["(" [param {"," param}] ")"] "=" expression
      lazy val subRoutineDefinition: Parser[FunctionDefinition] = rule {
        (%% << CL(SUB)) ~ commit(ident ~ (CL(LPAREN) >> (ident ~ typeAnnotation.?).repeat0By(CL(COMMA)) << CL(RPAREN)).? ~ lines) << CL(END) << SUB ^^ {
